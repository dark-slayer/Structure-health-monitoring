...
def load_mnist(mode='train', path='.'):
    ...
    if mode=="train":
    	data=[]
    	labels=[]
        cats=os.listdir("../data/small/withcrack/")
    	for cat in cats:
    	    imag=cv2.imread("../data/small/withcrack/"+cat)
            if imag is None:
                print "nothinghere"
            else:
                #print "somethinghere"
    	        img_from_ar = Image.fromarray(imag)
    	        resized_image = img_from_ar.resize((28,28))
                pimg = resized_image.convert('L')  # convert image to 8-bit grayscale
                WIDTH, HEIGHT = pimg.size
                
                pic = list(pimg.getdata()) # convert image data to a list of integers
# convert that to 2D list (list of lists of integers)
                #pic = [data[offset:offset+WIDTH] for offset in range(0, WIDTH*HEIGHT, WIDTH)]

            #resized_image.show()
    	        data.append(pic)
    	        labels.append(0)
        dogs=os.listdir("../data/small/withoutcrack/")
    	for dog in dogs:
            imag=cv2.imread("../data/small/withoutcrack/"+dog)
    	    if imag is None:
                print "nothinghere2"
            else:
                #print "somethinghere2"
                img_from_ar = Image.fromarray(imag)
                resized_image = img_from_ar.resize((28,28))
                pimg = resized_image.convert('L')  # convert image to 8-bit grayscale
                WIDTH, HEIGHT = pimg.size
...
# convert that to 2D list (list of lists of integers)
    	...
    	data=[]
        labels=[]
        cats=os.listdir("../data/small/withcrackt/")
        for cat in cats:
            imag=cv2.imread("../data/small/withcrackt/"+cat)
            if imag is None:
                print "nothinghere"
            else:
                #print "somethinghere"
...
# convert that to 2D list (list of lists of integers)
            ...
            #resized_image.show()
                data.append(pic)
                labels.append(0)
        dogs=os.listdir("../data/small/withoutcrackt/")
        for dog in dogs:
            imag=cv2.imread("../data/small/withoutcrackt/"+dog)
            if imag is None:
                print "nothinghere"
            else:
                #print "somethinghere"
